...
<div class="main-content">
    ...
    <div class="content-wrapper">
        ...
        <div class="page-header-custom mb-4">
            <div class="page-header-left">
                ...
                <div>
                    ...
                    <p class="page-subtitle">Perbarui banner yang tampil pada halaman depan sekolah.</p>
                </div>
            </div>
        ...
        <div class="custom-card">
            ...
            <form id="carousel-form" action="<?= site_url('admin/carousel/update/' . $carousel->id); ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="card-body-custom">
                    <div class="row g-4">
                        ...
                        <div class="col-md-8">
                            ...
                            <small class="field-hint"><i class="fa-solid fa-circle-info me-1"></i>Kosongkan jika tidak ingin mengganti foto.</small>
                            <input type="hidden" name="foto_lama" value="<?= html_escape($carousel->gambar); ?>">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <?php $isAktif = ($carousel->status === 'aktif'); ?>
                            <div class="status-toggle-card <?= $isAktif ? 'is-active' : ''; ?>" id="status-toggle" role="switch" tabindex="0" aria-checked="<?= $isAktif ? 'true' : 'false'; ?>">
                                <div class="status-toggle-icon">
                                    <i class="fa-solid <?= $isAktif ? 'fa-circle-check' : 'fa-circle-xmark'; ?>"></i>
                                </div>
                                <div class="status-toggle-text">
                                    <strong><?= $isAktif ? 'Aktif' : 'Nonaktif'; ?></strong>
                                    <span><?= $isAktif ? 'Banner akan ditampilkan' : 'Banner tidak ditampilkan'; ?></span>
                                </div>
                                <div class="status-toggle-switch"><span class="status-toggle-knob"></span></div>
                            </div>
                            <input type="hidden" name="status" id="status-input" value="<?= $isAktif ? 'aktif' : 'nonaktif'; ?>">
                        </div>

                        <!-- Preview -->
                        <div class="col-12">
                            <label class="form-label">Preview Gambar Saat Ini</label>
                            <div id="carousel-preview-container" class="carousel-preview-wrapper">
                                <img
...
.carousel-image-preview { display: block; width: min(560px, 100%); height: 210px; object-fit: cover; border-radius: 10px; border: 1px solid var(--carousel-border); }
...
@media (max-width: 768px) {
    ...
    .carousel-image-preview { width: 100%; height: auto; min-height: 160px; }
}
...
(function () {
    ...
    var defaultPreviewSrc = preview ? preview.getAttribute('src') : '';

    function showPreview(file) {
        if (!file) {
            if (preview && defaultPreviewSrc) {
