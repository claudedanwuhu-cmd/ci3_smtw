...
/**
 * Helper lokal: bentuk URL foto prestasi (logika sama dengan index.php).
...
// Foto saat ini (kosong jika belum ada foto)
$foto_url = prestasi_foto_url($prestasi->foto ?? '');
$has_foto = ($foto_url !== '');

// Tingkat: pertahankan nilai lama meski tidak ada di daftar standar
$tingkat_options = ['Sekolah', 'Kecamatan', 'Kabupaten', 'Provinsi', 'Nasional', 'Internasional'];
$tingkat_now = $prestasi->tingkat ?? '';
if ($tingkat_now !== '' && !in_array($tingkat_now, $tingkat_options, true)) {
    $tingkat_options[] = $tingkat_now;
}

// Format tanggal selalu Y-m-d agar input type="date" terisi benar
// (apa pun format asli dari database, mis. datetime "2024-05-10 00:00:00").
$tanggal_value = '';
if (!empty($prestasi->tanggal) && strpos($prestasi->tanggal, '0000') !== 0) {
    $ts_tanggal = strtotime($prestasi->tanggal);
    if ($ts_tanggal) {
        $tanggal_value = date('Y-m-d', $ts_tanggal);
    }
}
?>
...
<div class="main-content">
    ...
    <div class="content-wrapper">
        ...
        <div class="page-header-custom mb-4">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <h2 class="page-title">Edit Prestasi</h2>
        ...
        <!-- FORM INPUT EDIT -->
        <form id="form-edit-prestasi" data-loading-form action="<?= site_url('admin/prestasi/update/' . $prestasi->id); ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="custom-card mb-4">
                            ...
                            <div class="input-icon-group">
                                ...
                                <select id="tingkat" name="tingkat" class="form-select with-icon">
                                    ...
                                    <?php foreach ($tingkat_options as $opt): ?>
                                        <option value="<?= html_escape($opt); ?>" <?= ($tingkat_now === $opt) ? 'selected' : ''; ?>>
                                            <?= html_escape($opt); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                ...
                <div class="card-body-custom">
                    <div class="row g-4">
                        <div class="col-md-12">
                            <label class="form-label" id="foto-label">Upload Foto Prestasi <span class="label-hint">(Biarkan kosong jika tidak ingin mengganti)</span></label>

                            <!-- Nama foto lama, dikirim agar controller tahu foto yang sedang dipakai -->
                            <input type="hidden" name="foto_lama" value="<?= html_escape($prestasi->foto ?? ''); ?>">
                            ...
                            <div class="upload-dropzone" id="dropzone-area" role="button" tabindex="0" aria-labelledby="foto-label">
                                ...
                                <div class="upload-content text-center <?= $has_foto ? 'd-none' : ''; ?>" id="dropzone-prompt">
                                ...
                                <div class="preview-container text-center <?= $has_foto ? '' : 'd-none'; ?>" id="dropzone-preview">
                                    <img id="image-preview"
                                         src="<?= $has_foto ? html_escape($foto_url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'; ?>"
                                         alt="Preview Foto Prestasi" class="preview-thumb mb-2">
                                    <div class="d-flex align-items-center justify-content-center gap-2">
                                        <span class="preview-filename text-truncate" id="file-name"><?= $has_foto ? html_escape(basename($prestasi->foto)) : ''; ?></span>
                                        <span class="badge bg-primary fs-8">Ganti Foto</span>
                                    </div>
                                    <div class="preview-meta" id="file-size"><?= $has_foto ? 'Foto saat ini' : ''; ?></div>
                                    <button type="button" class="btn-remove-file d-none" id="btn-remove-file">
                                        <i class="fa-solid fa-rotate-left me-1"></i> Batalkan Perubahan
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="form-hint">
                                <i class="fa-solid fa-circle-info me-2"></i> Foto baru akan menggantikan foto lama setelah disimpan.
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-12">
                            ...
                            <textarea id="deskripsi" name="deskripsi" class="form-control custom-textarea" rows="4"
                                placeholder="Tuliskan keterangan singkat mengenai kegiatan, tingkat persaingan, atau catatan khusus..."><?= html_escape($prestasi->deskripsi ?? ''); ?></textarea>
                        </div>
                    </div>
                </div>
